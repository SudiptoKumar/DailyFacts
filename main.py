...
def self_test() -> None:
    ...
    # Deterministic image-ranking regression checks without network access.
    ...
    image_test_fact = type(sample)(
        fact_id="TEST", day=1, date_value="2026-01-01", fact="An octopus has three hearts.",
        slot=1, month="January", title="Three hearts in an octopus", category="Animals",
        evidence="Two hearts pump blood to the gills.", confidence="high",
        source_url="https://example.com", subcategory="Cephalopods", source_title="Example",
        source_domain="example.com", verification_status="verified",
    )
    image_score, _ = _candidate_score(image_test_fact, "octopus three hearts", candidate_page, candidate_info)
    assert image_score >= SETTINGS.image_min_score
    assert SETTINGS.bot_name in caption
    ...
    logger.info("Self-test passed with %d records and reference-style card output.", len(facts))
