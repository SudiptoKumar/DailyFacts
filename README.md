# Daily Facts Telegram Bot V2.1

**Channel:** https://t.me/FactsNewsroom  
**Bot:** Daily Facts

A dataset-first Telegram bot that publishes **20 unique facts per day** from 12 monthly CSV files. V2 adds a deterministic Unicode emoji system, category-aware headers, fact-specific subject emojis, and rotating context labels while keeping the factual database authoritative.
...
V2 uses ordinary Unicode emoji only. No Telegram custom-emoji entities are required for the post design.
...
𝗗𝗮𝗶𝗹𝘆 𝗙𝗮𝗰𝘁𝘀  #Topic #Subtopic
```

The renderer adds: 

- a category-aware header from `CATEGORY_ICONS`;
- up to two fact-specific subject emoji selected locally from title/fact/subcategory keywords;
- a stable rotating context label such as `💡 WHY IT'S INTERESTING`, `🔎 THE INTERESTING PART`, `📖 A LITTLE MORE CONTEXT`, or `🧩 HERE'S THE CATCH`;
- a fallback category emoji when no specific subject keyword is matched.

The subject/context selection is deterministic, so retries produce the same visual output. The system never calls an AI provider just to choose emoji.
...
Cerebras is used only for presentation enrichment:

- one concise `Why it's interesting` sentence
- two topical hashtags
- a branded footer with bold `Daily Facts` linked to `CHANNEL_URL` and the two topical hashtags on the same line

The CSV Title and Fact remain authoritative. Emoji selection is handled locally by `emoji_engine.py`, not by Cerebras.
...
The bot uses the Telegram Bot API. V2 keeps the visible emoji layer in ordinary Unicode text, so the design does not depend on Telegram custom emoji support.
...
The Telegram Bot API supports Rich Messages and the `sendRichMessage` method. V2.1 sends text-only rich posts using the documented `html` form of `InputRichMessage`. Image posts use `sendPhoto` with the same HTML caption, which avoids fragile local-file embedding in a rich-message block. If the primary path fails, the bot falls back to standard HTML `sendMessage`.
