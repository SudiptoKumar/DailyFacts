...
## V2.8 Publication State Architecture

Daily Facts uses deterministic two-batch scheduling:
...
The batch is selected by slot range, not by "first unpublished facts". This prevents the second run from selecting the first 10 facts again when the publication state is reset or missing.

Publication state is stored on a separate Git branch:
...
The project ZIP intentionally does not contain the authoritative publication ledger. The import workflow also excludes it, so importing a new ZIP cannot erase publication history.

Each successful post has an idempotency key:

```text
YYYY-MM-DD:FACT-ID
```

Batch receipts are stored per date. Batch 2 is blocked when Batch 1 has no valid receipt, preventing a state-reset scenario from accidentally publishing Batch 1 again.

### First deployment

Run the Daily Facts workflow manually with **Batch 1 + dry run disabled only when you are ready to publish**. The workflow automatically creates the `daily-facts-state` branch on the first successful state sync.

### Manual batch testing

```bash
python main.py --preview --date 2026-09-24 --batch 1
python main.py --preview --date 2026-09-24 --batch 2
python main.py --dry-run --date 2026-09-24 --batch 1 --max-posts 10
```

Do not copy `state/publication_state.json` into deployment ZIPs. It is runtime state, not source code.
