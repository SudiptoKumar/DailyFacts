...
def fallback_hashtags(f: Fact) -> list[str]:
    tags: list[str] = []
    for value in (f.category, f.subcategory):
        tag = _tagify(value)
        if tag and tag.lower() not in {x.lower() for x in tags}:
            tags.append(tag)
    if "#DailyFacts" not in {x.lower() for x in tags}:
        tags.append("#DailyFacts")
    return tags[:3]
...
def _clean_tags(values: list[str], f: Fact) -> list[str]:
    ...
    for fallback in fallback_hashtags(f):
        ...
        if len(tags) == 3:
            break
    if "#DailyFacts".lower() not in {x.lower() for x in tags}:
        tags = (tags[:2] + ["#DailyFacts"])[:3]
    return tags[:3]
...
- Hashtags: exactly 2 topical hashtags plus #DailyFacts. Do not use generic #Fact/#Facts.
...
Return JSON only."""
    ...
    try:
        ...
        if not title or not fact_text or not interesting or len(tags) != 3:
